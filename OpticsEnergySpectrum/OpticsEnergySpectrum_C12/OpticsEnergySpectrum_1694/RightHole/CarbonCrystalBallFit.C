...
//	// select the righ bottom hole
	std::string fiducials = Form("(%s.gold.th)<-0.023 &&(%s.gold.th)>-0.032 && (%s.gold.ph)<0.009 && (%s.gold.ph)>0.005",
				HRSArm.c_str(),HRSArm.c_str(),HRSArm.c_str(),HRSArm.c_str());
	...
	TF1 *fCtrxGaus=new TF1("fCtrxGaus","gaus",-0.154,-0.135);
	...
	fCtrxGausPar[3]=1.64;
	fCtrxGausPar[4]=1.16;
	TF1 *fCtrxCrystal=new TF1("fCtrxCrystal","crystalball",-0.25,fCtrxGaus->GetXmax());
	...
	TH1F *golddPhist = new TH1F("gold.dp", "gold.dp", 1000, -0.02, -0.01); //1676
    ...
    TF1 *fCgoldDpGaus=new TF1("fCgoldDpGaus","gaus",-0.0118,-0.0108);
    ...
    TF1 *fCgoldDpCrystal=new TF1("fCgoldDpCrystal","crystalball",-0.0170,fCgoldDpGaus->GetXmax());  // get the gaus fit for each individual peak, and use the paramater for the crystal ball
