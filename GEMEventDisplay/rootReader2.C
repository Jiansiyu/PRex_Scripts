...
/// add the code for the                                     // RGEM or left GEM
void rootReader2(TString fname="/home/newdriver/PRex/PRex_Data/GEMRootFile/prexRHRS_20862_00_test.root", std::string HRSarm="RGEM.rgems"){

	TCanvas *eventCanvas=new TCanvas(fname.Data(),"CanvasDisplay1",1000,1000);
	eventCanvas->Divide(1,2);
    eventCanvas->cd(1)->Divide(3,1);
	eventCanvas->cd(2)->Divide(3,1);
	TLine *detPlaneline[6];

	std::string GEMTreeNameHeader;
	std::string VDCTreeNameHeader;

	// will need to change the run number
	if(HRSarm=="RGEM.rgems"){
		GEMTreeNameHeader="RGEM.rgems";
		VDCTreeNameHeader="R";
	}else if (HRSarm=="LGEM.lgems") {
		GEMTreeNameHeader="LGEM.lgems";
		VDCTreeNameHeader="L";
	}else{
		std::cout<<"[ERROR]::"<<HRSarm.c_str()<< " CAN NOT FIND!!!!"<<std::endl
				<<"	=>system only take LGEM.lgems/RGEM.rgems"<<std::endl;
		exit(-1);
	}
	...
	if(PRex_GEM_tree->IsZombie()){
		...
		std::cout<<"Load file :: "<<fname.Data()<<std::endl;
		std::cout<<"Total Entries in the file:"<< (PRex_GEM_tree->GetEntries())<<std::endl;
	}
	...
	std::string fvdcXNumForm(Form("Ndata.%s.tr.x",VDCTreeNameHeader.c_str()));
	if(PRex_GEM_tree->GetListOfBranches()->Contains(fvdcXNumForm.c_str())){
		PRex_GEM_tree->SetBranchAddress(fvdcXNumForm.c_str(),&fvdcXNum);
	}else{
		std::cout<<"[Warning]::" << fvdcXNumForm.c_str() <<" did not find in the replay resuly"<<std::endl;
	}

	std::string fvdcXForm(Form("%s.tr.x",VDCTreeNameHeader.c_str()));
	if(PRex_GEM_tree->GetListOfBranches()->Contains(fvdcXForm.c_str())){
		PRex_GEM_tree->SetBranchAddress(fvdcXForm.c_str(),fvdcX);
	}else{
		std::cout<<"[Warning]:: "<<VDCTreeNameHeader.c_str()<<" did not find in the replay result"<<std::endl;
	}

	std::string fvdc_th_Form(Form("%s.tr.th",VDCTreeNameHeader.c_str()));
	if(PRex_GEM_tree->GetListOfBranches()->Contains(fvdc_th_Form.c_str())){
		PRex_GEM_tree->SetBranchAddress(fvdc_th_Form.c_str(),fvdc_th);
	}else{
		std::cout<<"[Warning]:: "<<fvdc_th_Form.c_str()<<" did not find in the replay result"<<std::endl;
	}

	std::string fvdc_ph_Form(Form("%s.tr.ph",VDCTreeNameHeader.c_str()));
	if(PRex_GEM_tree->GetListOfBranches()->Contains(fvdc_ph_Form.c_str())){
		PRex_GEM_tree->SetBranchAddress(fvdc_ph_Form.c_str(),fvdc_ph);
	}else{
		std::cout<<"[Warning]:: "<<fvdc_ph_Form.c_str()<<" did not find in the replay result"<<std::endl;
	}

	std::string fvdcYNumForm(Form("Ndata.%s.tr.y",VDCTreeNameHeader.c_str()));
	if(PRex_GEM_tree->GetListOfBranches()->Contains(fvdcYNumForm.c_str())){
		PRex_GEM_tree->SetBranchAddress(fvdcYNumForm.c_str(),&fvdcYNum);
	}else{
		std::cout<<"[Warning]:: "<<fvdcYNumForm.c_str()<<" did not find in the replay result"<<std::endl;
	}

	std::string fvdcYForm(Form("%s.tr.y",VDCTreeNameHeader.c_str()));
	if(PRex_GEM_tree->GetListOfBranches()->Contains(fvdcYForm.c_str())){
		PRex_GEM_tree->SetBranchAddress(fvdcYForm.c_str(),fvdcY);
	}else{
		std::cout<<"[Warning]:: "<<fvdcYForm.c_str()<<" did not find in the replay result"<<std::endl;
	}
	...
	// load the data for the theta-phi for GEM detectors
	std::string NGEMTracktheta_str(Form("Ndata.%sGEM.tr.th",VDCTreeNameHeader.c_str()));
	if(PRex_GEM_tree->GetListOfBranches()->Contains(NGEMTracktheta_str.c_str())){
		PRex_GEM_tree->SetBranchAddress(NGEMTracktheta_str.c_str(), &NGEMTracktheta);
	}else{
		std::cout<<"[Warning]::"<<NGEMTracktheta_str.c_str()<<"  CAN NOT find in the TREE"<<std::endl;

	}
	std::string GEMTracktheta_str(Form("%sGEM.tr.th",VDCTreeNameHeader.c_str()));
	if(PRex_GEM_tree->GetListOfBranches()->Contains(GEMTracktheta_str.c_str())){
		PRex_GEM_tree->SetBranchAddress(GEMTracktheta_str.c_str(), GEMTracktheta);
	}else{
		std::cout<<"[Warning]::"<<GEMTracktheta_str.c_str()<<"  CAN NOT find in the TREE"<<std::endl;

	}

	std::string NGEMTrackphi_str(Form("Ndata.%sGEM.tr.ph",VDCTreeNameHeader.c_str()));
	if(PRex_GEM_tree->GetListOfBranches()->Contains(NGEMTrackphi_str.c_str())){
		PRex_GEM_tree->SetBranchAddress(NGEMTrackphi_str.c_str(),&NGEMTrackphi);
	}else{
		std::cout<<"[Warning]::"<<NGEMTrackphi_str.c_str()<<"  CAN NOT find in the TREE"<<std::endl;
	}
	std::string GEMTrackphi_str(Form("%sGEM.tr.ph",VDCTreeNameHeader.c_str()));
	if(PRex_GEM_tree->GetListOfBranches()->Contains(GEMTrackphi_str.c_str())){
		PRex_GEM_tree->SetBranchAddress(GEMTrackphi_str.c_str(), GEMTrackphi);
	}else{
		std::cout<<"[Warning]::"<<GEMTrackphi_str.c_str()<<"  CAN NOT find in the TREE"<<std::endl;
	}
	...
	for (auto chamberID : chamberList){
		...
		// load the branch data
		// load the number of the Size X
		std::string NdetX_str(Form("Ndata.%s.x%d.hit.pos",GEMTreeNameHeader.c_str(),chamberID));
		if(PRex_GEM_tree->GetListOfBranches()->Contains(NdetX_str.c_str())){
				PRex_GEM_tree->SetBranchAddress(NdetX_str.c_str(),&GEMNDetX[chamberID]);
		}else{
				std::cout<<"[Warning]::"<<NdetX_str.c_str()<<" did not find in the replay resuly, skip it"<<std::endl;
			}

		// load the data
		GEMDetX[chamberID]=new double_t [100];
		std::string detX_str(Form("%s.x%d.hit.pos",GEMTreeNameHeader.c_str(),chamberID));
		if(PRex_GEM_tree->GetListOfBranches()->Contains(detX_str.c_str())){
				PRex_GEM_tree->SetBranchAddress(detX_str.c_str(),GEMDetX[chamberID]);
		}else{
				std::cout<<"[Warning]:: "<<detX_str.c_str()<<" did not find in the replay resuly, skip it"<<std::endl;
			}

		// load the Y dimension
		std::string NdetY_str(Form("Ndata.%s.y%d.hit.pos",GEMTreeNameHeader.c_str(),chamberID));
		if(PRex_GEM_tree->GetListOfBranches()->Contains(NdetY_str.c_str())){
				PRex_GEM_tree->SetBranchAddress(NdetY_str.c_str(),&GEMNDetY[chamberID]);
		}else{
				std::cout<<"[Warning]:: "<<NdetY_str.c_str()<<" did not find in the replay resuly, skip it"<<std::endl;
			}
		GEMDetY[chamberID]=new double_t [100];
		std::string detY_str(Form("%s.y%d.hit.pos",GEMTreeNameHeader.c_str(),chamberID));
		if(PRex_GEM_tree->GetListOfBranches()->Contains(detY_str.c_str())){
				PRex_GEM_tree->SetBranchAddress(detY_str.c_str(),GEMDetY[chamberID]);
		}else{
				std::cout<<"[Warning]:: "<<detY_str.c_str()<<" did not find in the replay resuly, skip it"<<std::endl;
			}


		// start load the coordi parameter in the root file
		std::string NCoordPosX_str(Form("Ndata.%s.x%d.coord.pos",GEMTreeNameHeader.c_str(),chamberID));
		if(PRex_GEM_tree->GetListOfBranches()->Contains(NCoordPosX_str.c_str())){
			PRex_GEM_tree->SetBranchAddress(NCoordPosX_str.c_str(), &GEM_NCoordPosX[chamberID]);
		}else{
			std::cout<<"[Warning]:: "<<NCoordPosX_str.c_str()<<" did not find in the replay resuly, skip it"<<std::endl;
		}
		GEM_CoordPosX[chamberID]=new double_t [100];
		std::string CoordPosX_str(Form("%s.x%d.coord.pos",GEMTreeNameHeader.c_str(),chamberID));
		if(PRex_GEM_tree->GetListOfBranches()->Contains(CoordPosX_str.c_str())){
			PRex_GEM_tree->SetBranchAddress(CoordPosX_str.c_str(), GEM_CoordPosX[chamberID]);
		}else{
			std::cout<<"[Warning]:: "<<CoordPosX_str.c_str()<<" data did not find in the replay resuly, skip it"<<std::endl;
		}

		std::string NCoordPosY_str(Form("Ndata.%s.y%d.coord.pos",GEMTreeNameHeader.c_str(),chamberID));
		if(PRex_GEM_tree->GetListOfBranches()->Contains(NCoordPosY_str.c_str())){
				PRex_GEM_tree->SetBranchAddress(NCoordPosY_str.c_str(), &GEM_NCoordPosY[chamberID]);
		}else{
				std::cout<<"[Warning]:: "<<NCoordPosY_str.c_str()<<" did not find in the replay resuly, skip it"<<std::endl;
		}

		GEM_CoordPosY[chamberID]=new double_t [100];
		std::string CoordPosY_str(Form("%s.y%d.coord.pos",GEMTreeNameHeader.c_str(),chamberID));
		if(PRex_GEM_tree->GetListOfBranches()->Contains(CoordPosY_str.c_str())){
			PRex_GEM_tree->SetBranchAddress(CoordPosY_str.c_str(), GEM_CoordPosY[chamberID]);
		}else{
			std::cout<<"[Warning]:: "<<CoordPosY_str.c_str()<<" did not find in the replay resuly, skip it"<<std::endl;
		}


		// start load the coordi parameter in the root file
		std::string NCoordTrackPosX_str(Form("Ndata.%s.x%d.coord.trkpos",GEMTreeNameHeader.c_str(),chamberID));
		if(PRex_GEM_tree->GetListOfBranches()->Contains(NCoordTrackPosX_str.c_str())){
			PRex_GEM_tree->SetBranchAddress(NCoordTrackPosX_str.c_str(), &GEM_NCoordTrackPosX[chamberID]);
		}else{
			std::cout<<"[Warning]:: "<<NCoordTrackPosX_str.c_str()<<" did not find in the replay resuly, skip it "<<__LINE__<<std::endl;
		}

		GEM_CoordTrackPosX[chamberID]=new double_t [100];
		std::string CoordTrackPosX_str(Form("%s.x%d.coord.trkpos",GEMTreeNameHeader.c_str(),chamberID));
		if(PRex_GEM_tree->GetListOfBranches()->Contains(CoordTrackPosX_str.c_str())){
			PRex_GEM_tree->SetBranchAddress(CoordTrackPosX_str.c_str(), GEM_CoordTrackPosX[chamberID]);
		}else{
			std::cout<<"[Warning]:: "<<CoordTrackPosX_str.c_str()<<" did not find in the replay resuly, skip it"<<std::endl;
		}

		std::string NCoordTrackPosY_str(Form("Ndata.%s.y%d.coord.trkpos",GEMTreeNameHeader.c_str(),chamberID));
		if(PRex_GEM_tree->GetListOfBranches()->Contains(NCoordTrackPosY_str.c_str())){
				PRex_GEM_tree->SetBranchAddress(NCoordTrackPosY_str.c_str(), &GEM_NCoordTrackPosY[chamberID]);
		}else{
				std::cout<<"[Warning]:: "<<NCoordTrackPosY_str.c_str()<<" did not find in the replay resuly, skip it"<<std::endl;
		}

		GEM_CoordTrackPosY[chamberID]=new double_t [100];
		std::string CoordTrackPosY_str(Form("%s.y%d.coord.trkpos",GEMTreeNameHeader.c_str(),chamberID));
		if(PRex_GEM_tree->GetListOfBranches()->Contains(CoordTrackPosY_str.c_str())){
			PRex_GEM_tree->SetBranchAddress(CoordTrackPosY_str.c_str(), GEM_CoordTrackPosY[chamberID]);
		}else{
			std::cout<<"[Warning]:: "<<CoordTrackPosY_str.c_str()<<" did not find in the replay resuly, skip it"<<std::endl;
		}
